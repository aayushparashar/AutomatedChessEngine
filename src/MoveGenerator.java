public class MoveGenerator {
	...
	/**
	 * CHESS BOARD Pawn = P/p Knight = K/k Bishop = B/b Rook = R/r Queen = Q/q King
	 * = A/a
	 */
	...
	public static String alphaBeta(int depth, int beta, int alpha, String move, int player) {
		...
		if (depth == 0 || list.length() == 0) {
			return move + (Evaluate.evaluate(list.length(), depth) * (2 * player - 1));
		}
	...
	public static void flipBoard() {
		...
		/*
		 * String temp; for(int i=0; i<32; i++) { int u = i/8; int v = i%8;
		 * if(Character.isLowerCase(chessBoard[u][v].charAt(0))) { temp =
		 * chessBoard[u][v].toUpperCase(); } else { temp =
		 * chessBoard[u][v].toLowerCase(); }
		 * 
		 * if(Character.isLowerCase(chessBoard[7-u][7-v].charAt(0))) { chessBoard[u][v]
		 * = chessBoard[7-u][7-v].toUpperCase(); } else { chessBoard[u][v] =
		 * chessBoard[7-u][7-v].toLowerCase(); } chessBoard[7-u][7-v] = temp; }
		 */
	...
	/**
	 * POSSIBLE MOVES String Format = x1y1x2y2capturedPiece Indicates a piece can
	 * move from x1,y1 to x2,y2 and captured piece is capturedPiece
	 */
