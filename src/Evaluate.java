...
public class Evaluate {
	...
	static int pawnBoard[][] = { { 0, 0, 0, 0, 0, 0, 0, 0 }, { 50, 50, 50, 50, 50, 50, 50, 50 },
			...
			for (int j = 0; j < 8; j++) {
				...
				if (MoveGenerator.chessBoard[i][j].equals("P")) {
					if (MATERIAL)
						material += pawnValue;
					if (POSITIONAL)
						positional += pawnBoard[i][j];
					MoveGenerator.kingAPos = i + "" + j;
					if (ATTACK)
						if (!MoveGenerator.isKingSafe(0, 0, 0, 0, MoveGenerator.chessBoard[0][0]))
							attack -= pawnValue / 2;
				} else if (MoveGenerator.chessBoard[i][j].equals("R")) {
					if (MATERIAL)
						material += rookValue;
					if (POSITIONAL)
						positional += rookBoard[i][j];
					MoveGenerator.kingAPos = i + "" + j;
					if (ATTACK)
						if (!MoveGenerator.isKingSafe(0, 0, 0, 0, MoveGenerator.chessBoard[0][0]))
							attack -= rookValue;
